...
function updateCart(cart) {
    for (var item in cart) {
        if (cart[item][2] > 0) {
            document.getElementById('s' + item).innerHTML = '<button class="btn btn-primary btnMinus" id= "minus' + item + '">-</button>' + '<span style="margin:0px 8px;">' + cart[item][2] + '</span>' + '<button class="btn btn-primary btnPlus" id= "plus' + item + '">+</button>';
        } else {
